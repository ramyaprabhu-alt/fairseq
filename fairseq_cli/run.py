...
def eval_lm(
    ...
    return {
        ...
        "per_forward_pass_attn" : per_forward_pass_attn,
        "per_forward_pass_ffn" : per_forward_pass_ffn,
        # "profiler" : p,
    }
...
#    print(model_overrides['moe_expert_count'])
    ...
    with open(file, 'a') as f:
        ...
        f.write('\n')
        f.write(str(sum([x[0] for x in results["per_forward_pass_attn"]])))
        f.write('\n')
        f.write("Per forward pass ffn :  ")
        # f.write(str(len(results["per_forward_pass_ffn"][0])))
        f.write("   ")
        ...
        f.write('\n')
        f.write(str(sum([x[0] for x in results["per_forward_pass_ffn"]])))
        f.write('\n')
